...
`;class F extends g{constructor(){super(...arguments),this.handleKeyDown=e=>{var t;if(e.key===((t=this.props.key)==null?void 0:t.toLowerCase())){const o=this.props.active!=="true";this.setState({isActive:o}),this.dispatchEvent(new CustomEvent("change",{detail:o}))}}}onMount(){window.addEventListener("keydown",this.handleKeyDown)}onUmount(){window.removeEventListener("keydown",this.handleKeyDown)}render(){const{key:e,active:t}=this.props;return c("div",null,c("style",null,z),c("div",{class:N({active:t==="true"})},"[",e==null?void 0:e.toUpperCase(),"] ",c("slot",null)))}}customElements.define("flip-switch",F);const _=`
  .cell {
    text-align: center;
    height: 25px;
    width: 25px;
...
`;class $ extends g{render(){const{val:e}=this.props;return c("div",null,c("style",null,_),c("div",{class:N("cell",e==null?void 0:e.toUpperCase())},(e==null?void 0:e.toUpperCase())||0,","))}}customElements.define("tetris-cell",$);const Y=`
  .row {
    line-height: 25px;
    display: flex;
  }
`;class W extends g{render(){const{last:e}=this.props;return c("div",null,c("style",null,Y),c("div",{class:"row"},"[",this.children,"] ",e==="true"?"]":","))}}customElements.define("tetris-row",W);const X=`
  .container {
    display: flex;
  }
  .grid-start {
...
`;class Q extends g{render(){const{grid:e}=this.props;return c("div",{id:"test"},c("style",null,X),c("div",{class:"container"},c("div",{class:"grid-start"},"var grid = [\xA0"),c("div",null,JSON.parse(e).map((t,o)=>c("tetris-row",null,t.map(n=>c("tetris-cell",{val:String(n)})))))))}}customElements.define("tetris-grid",Q);const U=()=>{const r=Object.keys(A),e=r[Math.floor(Math.random()*r.length)];return{x:6,y:0,rotation:0,blockCode:e}},b=()=>{const r=new Array(m.gridSize.row).fill(0).map(()=>new Array(m.gridSize.col).fill(0));return J({block:U(),grid:r,stopped:!0,score:0},(e,{grid:t,block:o,score:n,stopped:s})=>{var i;switch(e){case"grid":case"block":{const a=document.querySelector("#grid");if(!a)return;const d=x({grid:t,block:o});a.innerHTML="";const y=c("tetris-grid",{grid:JSON.stringify(d)});a.appendChild(y);break}case"score":{const a=document.querySelector("#score");if(!a)return;a.innerText=n.toString();break}case"stopped":{s?(clearInterval(f),f=void 0):f=setInterval(k,m.speed),(i=document.querySelector("#stop"))==null||i.setAttribute("active",s?"true":"false");break}}})};let f,l=b();const u=r=>{try{const e=p(p({},l.block),r);return x({grid:l.grid,block:e}),l.block=e,!0}catch{return!1}},Z=()=>{clearInterval(f),f=void 0,f=setInterval(k,m.speed)},k=()=>{if(l.score+=1,u({y:l.block.y+1}))return;const{grid:r,block:e}=l,{grid:t,removedLine:o}=V(x({grid:r,block:e}));u(U())?(l.score+=o*100,l.grid=t):l=b()};window.onload=()=>{var r,e;l=b(),(r=document.querySelector("#restart"))==null||r.addEventListener("change",t=>{t instanceof CustomEvent&&(l=b())}),(e=document.querySelector("#stop"))==null||e.addEventListener("change",t=>{t instanceof CustomEvent&&(l.stopped=t.detail)})};document.addEventListener("keydown",r=>{if(l.stopped)return;const{block:{rotation:e,x:t,y:o}}=l;switch(r.key){case"ArrowLeft":return u({x:t-1});case"ArrowRight":return u({x:t+1});case"ArrowDown":return u({y:o+1});case"ArrowUp":return u({rotation:e>=3?0:e+1});case" ":{let n=!0,s=o;do n=u({y:s+=1});while(n);Z(),k();break}}});
